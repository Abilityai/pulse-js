...
async function main() {
  const claude = new Chat({ model: 'claude-3-haiku-20240307' });

  // Simple completion test
  console.log('\nTesting simple completion...');
  const response = await claude.get({
    messages: [
  ...
  const response3 = await claude.get({
    messages: [
